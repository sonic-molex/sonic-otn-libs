...
public:
    ...
    virtual void
    Format(std::ostringstream &ss, const std::string& fmt)
    {
        ss << m_arg;
    }
private:
...
static void
FormatItem(std::ostringstream& ss, const std::string& item, const ArgArray& args)
{
    int index = 0;
    int alignment = 0;
    std::string fmt;
...
template <class T>
static void
Transfer(ArgArray& argArray, T t)
{
...
static void
Transfer(ArgArray& argArray, T t, Args && ... args)
{
    Transfer(argArray, t);
